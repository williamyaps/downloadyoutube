...
// @name:zh           网页限制解除作者叶威廉
// @name:zh-CN        网页限制解除作者叶威廉
// @name:zh-TW        網頁限制解除作者葉威廉
// @name:ja           ページ上の規制緩和は、ウィリアムを残します
...
// a function that return true and if else
...
// Mendapatkan semua dari elements document
...
// Gets the rules that should be used for the target domain name
...
// initialization
function init() {
  // Pengaturan aturan domain
  var url = window.location.host + window.location.pathname;
  var rule = getRule(url);

  // Mengatur daftar event
  hook_eventNames = rule.hook_eventNames.split("|");
  // TODO Allowed to return value
  unhook_eventNames = rule.unhook_eventNames.split("|");
  eventNames = hook_eventNames.concat(unhook_eventNames);

  // menghapus DOMO Event
  if(rule.dom0) {
